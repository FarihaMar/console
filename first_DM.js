(function() {
  ...
  padding-top:1px;
      ...
      .comment-btn::after {
        display: none; /* Removed the ripple circle effect */
      }
  ...
  `;       
    ...
    stopButton.onclick = () => {
      ...
      // Hide loading container after 1 second
      setTimeout(() => {
  ...
  (async function() {
    ...
    console.log('Ready to message profiles', 0);
    ...
    async function getProfileDataWithCache() {
      console.log('Checking profile data cache', 0);
      ...
      if (cachedData) {
        console.log("Using cached profile data", 0);
        return cachedData;
      }
      
      // If no cache, scrape fresh data
      console.log("Gathering fresh profile data", 2);
    ...
    async function initializeQuickReplyButton() {
      ...
      console.log('Initializing quick reply buttons', 1);
      ...
      if (!buttonConfigs.length) {
        console.log('No button configs found', 0);
        return;
      }
      ...
      if (!inserted) {
        console.log('Could not find insertion point', 0);
      ...
      buttonConfigs.forEach(config => {
        ...
        button.addEventListener('click', async () => {
          ...
          try {
            console.log(`Starting ${config.name} message`, 0);
            ...
            try {
              console.log('Generating AI message', 2);
              ...
              console.log('Opening message window', 1);
              ...
              if (error.message.includes('cancelled') || error.message.includes('aborted')) {
                loadingMessage.textContent = '❌ Generation cancelled';
                console.log('Message generation cancelled', 0);
              } else {
                console.error(error);
            ...
            console.log('Message ready to send!', 1);
          } catch (error) {
            console.error(error);
            console.log(`Error: ${error.message}`, 0);
          } finally {
      ...
      isInitialized = true;
      console.log('Quick reply buttons initialized', 0);
    }
    ...
    setTimeout(() => {
      if (!isInitialized) {
        console.log('Running fallback initialization', 0);
        initializeQuickReplyButton();
      }
    }, 3000);
    ...
    setInterval(() => {
      if (window.location.href.includes('linkedin.com/in/') && window.location.href !== lastUrl) {
        console.log('Detected profile page change', 0);
        lastUrl = window.location.href;
        isInitialized = false;
        initializeQuickReplyButton();
      }
  ...
  let statusMessages = [];
  let isPopupVisible = false; // Changed to false to hide by default
    ...
    header.innerHTML = `
      ...
      <div>
        ...
        <button id="toggle-status" style="
          ...
          cursor: pointer;
          transition: all 0.2s ease;
        ">SHOW</button> <!-- Changed to SHOW since we're hiding by default -->
      </div>
    `;
